...
public class ServicoControle {
    ...
    @GetMapping("/listarServicosPorEmpresa/{idEmpresa}")
    public ResponseEntity<?> listarServicosPorEmpresa(@PathVariable Long idEmpresa) {
        Optional<Empresa> empresaOpt = repositorioEmpresa.findById(idEmpresa);
        if (empresaOpt.isPresent()) {
            Empresa empresa = empresaOpt.get();
            List<Servico> servicos = empresa.getServicos();
            if (!servicos.isEmpty()) {
                adicionadorLinkServico.adicionarLink(servicos);
                return new ResponseEntity<>(servicos, HttpStatus.OK);
            } else {
                return new ResponseEntity<>("Nenhum serviço encontrado para esta empresa.", HttpStatus.NOT_FOUND);
            }
        } else {
